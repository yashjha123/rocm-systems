...
// on physical gfx1201 (all 2^32 inputs) and every V_EXP_F16 output under the
// default MODE. It is derived from hardware outputs and mirrors their observed
...
// The FP32 result rounds A to 24 bits with round-to-nearest-even; F16 results
// round the same unrounded A directly, which differs from double rounding.
...
/// @brief AMD half-precision 2^x matching physical gfx1201 V_EXP_F16 under default MODE.
/// @details The exactly promoted source feeds the same unrounded evaluation as FP32; the
/// result is rounded once to F16 (nearest-even, subnormals kept). Guest rounding mode is
/// ignored by hardware.
inline uint16_t amdgpu_exp_f16_bits(uint16_t value) {
  ...
  if (exponent == 0x1f) {
    ...
    int32_t shift = std::countl_zero(mantissa) - 21; // normalize subnormal to 1.x
    mantissa = (mantissa << shift) & 0x3ffu;
    bits = sign | (static_cast<uint32_t>(113 - shift) << 23) | (mantissa << 13);
  }
  return detail::amdgpu_exp_round_f16(detail::amdgpu_exp_wide(bits));
}
